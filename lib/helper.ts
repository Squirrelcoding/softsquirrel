import { PaperLink, PaperType, PostLink } from "./types";
      ...
      Object.keys(data).forEach(key => {
        ...
        switch(data[key].type) {
          ...
          case "Computer Science":
            data[key].type = PaperType.ComputerScience;
            break;
          default:
...
export const getDoc = async <T> (collection: string, doc: string, converter: Converter<T>) => {

    const db = getDbHandler();

    const ref = db.collection(collection).doc(doc);

    const data = (await ref.withConverter(converter).get()).data()!;
